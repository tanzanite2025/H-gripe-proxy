...
import (
	...
	"github.com/tanzanite2025/mihomo-optimized/common/orderedmap"
	...
	"github.com/tanzanite2025/mihomo-optimized/component/cidr"
	"github.com/tanzanite2025/mihomo-optimized/component/fakeip"
	...
	snifferTypes "github.com/tanzanite2025/mihomo-optimized/constant/sniffer"
	"github.com/tanzanite2025/mihomo-optimized/dns"
	"github.com/tanzanite2025/mihomo-optimized/listener"
	...
	RP "github.com/tanzanite2025/mihomo-optimized/rules/provider"
	RW "github.com/tanzanite2025/mihomo-optimized/rules/wrapper"
	T "github.com/tanzanite2025/mihomo-optimized/tunnel"
...
func ParseRawConfig(rawCfg *RawConfig) (*Config, error) {
	...
	go func() {
		defer p1wg.Done()
		v, err := parseGeneral(rawCfg)
		p1mu.Lock()
		if err != nil && p1err == nil {
			p1err = err
		}
		p1.general = v
		p1mu.Unlock()
	...
	go func() {
		defer p1wg.Done()
		v, err := parseExperimental(rawCfg)
		p1mu.Lock()
		if err != nil && p1err == nil {
			p1err = err
		}
	...
	config.General = p1.general
	config.Controller = p1.controller
	config.Experimental = p1.experimental
	config.IPTables = p1.iptables
	config.NTP = p1.ntpCfg
	config.Profile = p1.profile
	config.TLS = p1.tlsCfg
	...
	// Phase 3: 并行解析 hosts, sniffer (无互相依赖)
	var (
		hosts      *trie.DomainTrie[resolver.HostValue]
		snifferCfg *sniffer.Config
		p3err      error
		p3mu       sync.Mutex
		p3wg       sync.WaitGroup
	)
	...
	go func() {
		defer p3wg.Done()
		v, err := parseHosts(rawCfg)
		p3mu.Lock()
		if err != nil && p3err == nil {
			p3err = err
		}
		hosts = v
		p3mu.Unlock()
	...
	go func() {
		defer p3wg.Done()
		v, err := parseSniffer(rawCfg.Sniffer, ruleProviders)
		p3mu.Lock()
		if err != nil && p3err == nil {
			p3err = err
		}
...
func parseRules(rulesConfig []string, proxies map[string]C.Proxy, ruleProviders map[string]P.RuleProvider, subRules map[string][]C.Rule, format string) ([]C.Rule, error) {
	...
	for idx, line := range rulesConfig {
		...
		if format == "rules" { // only wrap top level rules
			...
			parsed = RW.NewRuleWrapper(parsed, source)
		}
