...
func dnsRouter() http.Handler {
	...
	r.Get("/metrics", getDnsMetrics)
	r.Post("/warmup", warmupDns)
	return r
}
...
func getDnsMetrics(w http.ResponseWriter, r *http.Request) {
	render.JSON(w, r, dnsMetrics.GetDnsMetrics().GetAllStats())
}

func warmupDns(w http.ResponseWriter, r *http.Request) {
	go dnsMetrics.GetPreResolver().Warmup()
