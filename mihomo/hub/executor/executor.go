...
import (
	...
	"github.com/tanzanite2025/mihomo-optimized/component/profile"
	...
	"github.com/tanzanite2025/mihomo-optimized/listener/tproxy"
	"github.com/tanzanite2025/mihomo-optimized/log"
	"github.com/tanzanite2025/mihomo-optimized/ntp/ntp"
	"github.com/tanzanite2025/mihomo-optimized/tunnel"
)

var mux sync.Mutex

var perfInitOnce sync.Once
...
func ApplyConfig(cfg *config.Config, force bool) {
	...
	// Apply performance optimizations once on first config load
	perfInitOnce.Do(func() {
		perf.Init()
		perf.DefaultHotReloader.SetConfigUpdateCallback(func(old, new_ string) {
			log.Infoln("[HotReload] config updated")
		})
		perf.DefaultHotReloader.SetRuleUpdateCallback(func(version int64) {
			log.Infoln("[HotReload] rules updated to version %d", version)
		})
	})
...
func updateDNS(c *config.DNS, generalIPv6 bool) {
	...
	// Evaluate DNS server trust levels
	evaluator := dns.GetTrustEvaluator()
	allServers := make([]string, 0)
	for _, ns := range c.NameServer {
		allServers = append(allServers, ns.Addr)
	}
	...
	evaluator.EvaluateServers(allServers)

	// Warm up DNS cache with commonly used domains
	go dns.GetPreResolver().Warmup()
}
...
//go:linkname temporaryUpdateGeneral github.com/tanzanite2025/mihomo-optimized/config.temporaryUpdateGeneral
