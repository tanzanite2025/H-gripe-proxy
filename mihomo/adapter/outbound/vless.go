...
import (
	...
	"github.com/tanzanite2025/mihomo-optimized/transport/vless"
	...
	"github.com/tanzanite2025/mihomo-optimized/transport/xhttp"
...
type VlessOption struct {
	...
	AntiDPIOpts       AntiDPIOption     `proxy:"anti-dpi-opts,omitempty"`
}
...
func (v *Vless) streamConnContext(ctx context.Context, c net.Conn, metadata *C.Metadata) (conn net.Conn, err error) {
	...
	// Apply anti-DPI wrapping if enabled
	if v.option.AntiDPIOpts.Enabled && conn != nil {
		conn = vmess.NewAntiDPIConn(conn, v.antiDPIConfig())
	}
	return
}

func (v *Vless) antiDPIConfig() *vmess.AntiDPIConfig {
	opts := v.option.AntiDPIOpts
	cfg := &vmess.AntiDPIConfig{
		Enabled:      opts.Enabled,
		PaddingMode:  opts.PaddingMode,
		MinPadding:   opts.MinPadding,
		MaxPadding:   opts.MaxPadding,
		JitterMs:     opts.JitterMs,
		BurstBefore:  opts.BurstBefore,
		DummyTraffic: opts.DummyTraffic,
	}
	if cfg.PaddingMode == "" {
		cfg.PaddingMode = "random"
	}
	if cfg.MaxPadding == 0 {
		cfg.MaxPadding = 64
	}
