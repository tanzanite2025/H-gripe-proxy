...
import (
	...
	wgopt "github.com/tanzanite2025/mihomo-optimized/transport/wireguard"
...
type WireGuard struct {
	*Base
	...
	// Optimization: MTU auto-detection and health monitoring
	mtuDetector   *wgopt.MTUDetector
	healthMonitor *wgopt.ConnectionHealthMonitor
}
...
func NewWireGuard(option WireGuardOption) (*WireGuard, error) {
	...
	// Initialize MTU detector and health monitor
	outbound.mtuDetector = wgopt.NewMTUDetector(option.MTU)
	outbound.healthMonitor = wgopt.NewConnectionHealthMonitor(func() {
		// On unhealthy: reset init state to force reconnection
		outbound.initOk.Store(false)
		log.Warnln("[WG](%s) connection unhealthy, will reconnect on next use", option.Name)
	})
...
func (w *WireGuard) init0(ctx context.Context) error {
	...
	// Start health monitoring and trigger async MTU probe
	w.healthMonitor.Start()
	go w.mtuDetector.ProbeMTU(context.Background(), func(ctx context.Context, network, addr string) (net.Conn, error) {
		return w.dialer.DialContext(ctx, network, addr)
	})
