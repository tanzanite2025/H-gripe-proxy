...
func GetFingerprint(clientFingerprint string) (UClientHelloID, bool) {
	...
	if clientFingerprint == "rotating" {
		helloID := obfuscation.DefaultTLSRotator.CurrentHelloID()
		name := obfuscation.DefaultTLSRotator.CurrentFingerprint()
		obfuscation.DefaultTLSFingerprintStats.Record(name)
		log.Debugln("use rotating HelloID:%s", name)
		return helloID, true
	}
