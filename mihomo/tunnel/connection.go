...
func handleSocket(inbound, outbound net.Conn) {
	// Apply obfuscation wrapping to the outbound (proxy) side
	obfsOutbound := obfuscation.DefaultEngine.WrapConn(outbound)
	N.Relay(inbound, obfsOutbound)
}

// handleSocketWithStats relays with connection manager tracking.
func handleSocketWithStats(inbound, outbound net.Conn, inboundID, outboundID string) {
	obfsOutbound := obfuscation.DefaultEngine.WrapConn(outbound)
	RelayWithStats(inbound, obfsOutbound, inboundID, outboundID, DefaultConnManager)
}
