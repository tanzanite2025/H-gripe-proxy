...
import (
	...
	"github.com/tanzanite2025/mihomo-optimized/component/loopback"
	"github.com/tanzanite2025/mihomo-optimized/component/nat"
	...
	"github.com/tanzanite2025/mihomo-optimized/component/sniffer"
	...
	"github.com/tanzanite2025/mihomo-optimized/tunnel/statistic"
)
...
var (
	...
	// Rule match cache for performance optimization
	ruleMatchCache = newRuleCache(2000)
)
...
func UpdateRules(newRules []C.Rule, newSubRule map[string][]C.Rule, rp map[string]P.RuleProvider) {
	...
	subRules = newSubRule
	configMux.Unlock()
	ruleMatchCache.Invalidate()

	// Notify hot reloader about rule update
	perf.DefaultHotReloader.UpdateRules()
}

// InsertPosition controls where a runtime rule is inserted.
type InsertPosition int

const (
	PositionAppend  InsertPosition = iota // default: append to end
	PositionPrepend                       // insert at beginning
)

// InsertRule inserts a new rule into the rules list.
// position controls placement: PositionAppend (default) or PositionPrepend.
// Returns the index of the inserted rule.
func InsertRule(rule C.Rule, position ...InsertPosition) int {
	pos := PositionAppend
	if len(position) > 0 {
		pos = position[0]
	}
	...
	case PositionPrepend:
		rules = append([]C.Rule{rule}, rules...)
		configMux.Unlock()
		return 0
	default:
...
// InsertSubRule inserts a new rule into the named sub-rule list.
// If the sub-rule list does not exist, it is created.
// position controls placement: PositionAppend (default) or PositionPrepend.
// Returns the index of the inserted rule within the sub-rule list.
func InsertSubRule(name string, rule C.Rule, position ...InsertPosition) int {
	pos := PositionAppend
	if len(position) > 0 {
		pos = position[0]
	}
	...
	ruleMatchCache.Invalidate()
	if subRules == nil {
		subRules = make(map[string][]C.Rule)
	}
	sr := subRules[name]
	switch pos {
	case PositionPrepend:
		sr = append([]C.Rule{rule}, sr...)
		subRules[name] = sr
		configMux.Unlock()
		return 0
	default:
...
// DeleteSubRuleBySource soft-deletes all rules in the named sub-rule list
// whose RuleWrapper source matches the given prefix.
func DeleteSubRuleBySource(name string, sourcePrefix string) int {
	configMux.Lock()
	count := 0
	sr, ok := subRules[name]
	if !ok {
	...
	for _, rule := range sr {
		if rw, ok := rule.(C.RuleWrapper); ok {
			if strings.HasPrefix(rw.Source(), sourcePrefix) {
				rw.SetDeleted(true)
				count++
			}
		}
...
// SubRules returns a copy of the current sub-rules map.
func SubRules() map[string][]C.Rule {
	configMux.RLock()
	defer configMux.RUnlock()
	cp := make(map[string][]C.Rule, len(subRules))
...
func handleTCPConn(connCtx C.ConnContext) {
	...
	defer func(remoteConn C.Conn) {
		_ = remoteConn.Close()
		// Record disconnection in traffic stats
		DefaultConnTrafficStats.Untrack(connTrackID)
		// Record rule traffic disconnection
		if rule != nil {
			ruleKey := rule.RuleType().String() + "/" + rule.Payload()
			DefaultRuleTrafficStats.RecordDisconnection(ruleKey)
		}
	}(remoteConn)

	DefaultConnTrafficStats.Track(connTrackID, metadata.RemoteAddress(), remoteConn.Chains().String())
	DefaultConnManager.Track(connTrackID, remoteConn, metadata, 0)

	// Record rule traffic
	if rule != nil {
		ruleKey := rule.RuleType().String() + "/" + rule.Payload()
		DefaultRuleTrafficStats.Record(ruleKey, rule.RuleType().String(), rule.Payload(), int64(peekLen), 0)
		DefaultRuleTrafficStats.RecordConnection(ruleKey)
	}
...
func match(metadata *C.Metadata, helper C.RuleMatchHelper) (C.Proxy, C.Rule, error) {
	...
	// Fast path: check rule match cache
	if proxy, rule, ok := ruleMatchCache.Get(metadata); ok {
		// Record hit for statistics even on cache hit
		if wrapper, ok2 := rule.(C.RuleWrapper); ok2 {
			wrapper.Hit()
		}
	...
	for _, rule := range getRules(metadata) {
		if matched, ada := rule.Match(metadata, helper); matched {
			...
			ruleMatchCache.Put(metadata, adapter, rule)
