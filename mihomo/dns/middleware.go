...
func newHandler(resolver resolver.Resolver, mapper *ResolverEnhancer) handler {
	...
	// Add DNS fingerprint protection (modifies outgoing queries before resolver)
	middlewares = append(middlewares, withFingerprintProtection(true))

	// Add pollution detection (checks incoming responses after resolver)
	// Must be first in the list so it wraps the entire chain
	middlewares = append([]middleware{withPollutionDetection()}, middlewares...)
