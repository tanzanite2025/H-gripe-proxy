...
fn extract_backup_archive(file: std::fs::File, destination: PathBuf) -> Result<()> {
    ...
    for index in 0..zip.len() {
        ...
        let relative_path = entry
            .enclosed_name()
            .ok_or_else(|| anyhow!("Invalid backup archive entry: {entry_name}"))?
            .to_path_buf();
        let output_path = destination.join(&relative_path);
        if !output_path.starts_with(&destination) {
            return Err(anyhow!("Invalid backup archive entry: {entry_name}"));
        }
        if entry.is_dir() {
