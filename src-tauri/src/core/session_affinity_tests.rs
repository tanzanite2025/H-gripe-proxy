...
async fn test_disabled_session_affinity() {
    ...
    // 应该直接返回第一个节点，不创建绑定
    let node = manager.select_node_for_domain("chat.openai.com", &nodes).await.unwrap();
...
async fn test_runtime_domain_rule_binding_overrides_existing_domain_binding() {
    ...
    let first = manager.select_node_for_domain("chat.openai.com", &nodes).await.unwrap();
    ...
    let second = manager.select_node_for_domain("chat.openai.com", &nodes).await.unwrap();

    assert_eq!(second, "node2");

    let bindings = manager.get_all_bindings().await.unwrap();
    assert!(bindings.iter().any(|binding| {
        binding.binding_type == "domain-rule" && binding.key == "rule:*.openai.com" && binding.node_id == "node2"
    }));

    manager.clear_domain_binding("rule:*.openai.com").await.unwrap();

    let cleared_bindings = manager.get_all_bindings().await.unwrap();
    assert!(
        !cleared_bindings
            .iter()
            .any(|binding| { binding.binding_type == "domain-rule" && binding.key == "rule:*.openai.com" })
    );
}
