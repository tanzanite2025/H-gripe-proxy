...
async fn build_mihomo_fallback_retirement_execution_report(
    ...
    Ok(MihomoFallbackRetirementExecutionReport {
        ...
        warnings: vec![
            ...
            "Mihomo remains fallback for SOCKS UDP, unsupported encrypted protocols, packet capture, route install, and full binary removal".into(),
        ],
        facts: vec![
...
fn retained_fallback_scope() -> Vec<String> {
    vec![
        ...
        "unsupported DNS features beyond bounded fake-ip allocation: fake-ip cache/reverse mapping, fallback-filter, nameserver-policy".into(),
