...
fn retained_fallback_scope() -> Vec<String> {
    vec![
        ...
        "unsupported DNS features beyond bounded fake-ip/fallback-filter/nameserver-policy and policy/cache/upstream bundle execution: default DNS runtime ownership, live resolver replacement, full GeoIP database loading, production persistent cache storage, and geodata refresh".into(),
