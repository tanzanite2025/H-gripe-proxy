...
async fn verify_default_runtime_execution_guard_metadata(
    guard: &DnsDefaultRuntimeOptInExecutionGuardReport,
    errors: &mut Vec<String>,
) -> bool {
...
async fn read_default_runtime_guard_yaml<T: DeserializeOwned>(
    path: Option<&str>,
    label: &str,
    errors: &mut Vec<String>,
) -> Option<T> {
...
async fn read_default_runtime_active_state(errors: &mut Vec<String>) -> Option<DnsDefaultRuntimeActiveState> {
...
async fn read_default_runtime_execution_record_from_active(
    active_state: Option<&DnsDefaultRuntimeActiveState>,
    errors: &mut Vec<String>,
...
async fn persist_default_runtime_execution_record(
...
async fn persist_default_runtime_active_state(
...
fn default_runtime_state_dir() -> Result<std::path::PathBuf> {
    Ok(dirs::app_runtime_dir()?.join("dns-default-runtime"))
}

fn default_runtime_active_state_path() -> Result<std::path::PathBuf> {
    Ok(default_runtime_state_dir()?.join("active-runtime.yaml"))
}

fn default_runtime_execution_record_path(event_id: &str) -> Result<std::path::PathBuf> {
...
fn default_runtime_post_execution_status_label(
    status: DnsDefaultRuntimePostExecutionVerificationStatus,
) -> &'static str {
    match status {
...
fn default_runtime_epoch_seconds() -> u64 {
    SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .unwrap_or_default()
        .as_secs()
