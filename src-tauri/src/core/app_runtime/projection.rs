...
pub async fn apply_app_runtime_projection_artifact_to_runtime(
    ...
    let result = async {
        ...
        let outcome = runtime_lifecycle::update_runtime_config_through_restart_boundary(
            request.force,
    ...
    if result.is_err() {
        let _ =
            runtime_lifecycle::update_runtime_config_through_restart_boundary(true, "app-runtime-projection-rollback")
                .await;
    }
...
pub async fn rollback_app_runtime_projection_activation() -> Result<AppRuntimeStateDocument> {
    ...
    if current.mutates_runtime {
        let outcome = runtime_lifecycle::update_runtime_config_through_restart_boundary(
            true,
            "app-runtime-projection-drift-replay",
        )
        .await?;
        if !outcome.is_valid() {
