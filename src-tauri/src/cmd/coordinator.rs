...
pub fn coordinator_initialize() -> CmdResult<()> {
    crate::core::coordinator::get_coordinator().initialize().stringify_err()
}

/// 关闭协调器
#[tauri::command]
pub fn coordinator_shutdown() -> CmdResult<()> {
    crate::core::coordinator::get_coordinator().shutdown().stringify_err()
}

/// 获取高级配置（从 coordinator 内存缓存读取）
#[tauri::command]
pub fn get_advanced_config() -> CmdResult<AdvancedConfig> {
    Ok(crate::core::coordinator::get_coordinator().get_advanced_config())
}

/// 保存高级配置
#[tauri::command]
pub async fn save_advanced_config(config: AdvancedConfig) -> CmdResult<()> {
    crate::core::coordinator::save_advanced_config(&config)
        .await
        .stringify_err()
}
