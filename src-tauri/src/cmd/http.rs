...
use crate::http::{
    HeaderSanitizationConfig, HeaderSanitizer, BrowserTemplate, BrowserFingerprint,
};
...
static HEADER_SANITIZER: Lazy<Arc<RwLock<HeaderSanitizer>>> = Lazy::new(|| {
    Arc::new(RwLock::new(HeaderSanitizer::new(
        HeaderSanitizationConfig::default(),
    )))
});
...
/// 测试 HTTP 头净化效果
#[tauri::command]
pub fn header_sanitization_test(
...
/// 获取指定浏览器模板的指纹
#[tauri::command]
pub fn header_sanitization_get_fingerprint(
    template: String,
