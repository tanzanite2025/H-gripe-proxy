...
pub async fn traffic_obfuscation_get_config() -> CmdResult<TrafficObfuscationConfig> {
    Ok(crate::feat::traffic_obfuscation_get_config().await)
}
...
pub async fn traffic_obfuscation_start() -> CmdResult<()> {
    crate::feat::traffic_obfuscation_start().await.stringify_err()
}

/// 停止流量混淆
#[tauri::command]
pub async fn traffic_obfuscation_stop() -> CmdResult<()> {
    crate::feat::traffic_obfuscation_stop().await.stringify_err()
}

/// 获取流量混淆统计
#[tauri::command]
pub async fn traffic_obfuscation_get_stats() -> CmdResult<ObfuscationStats> {
    Ok(crate::feat::traffic_obfuscation_get_stats().await)
}

/// 重置流量混淆统计
#[tauri::command]
pub async fn traffic_obfuscation_reset_stats() -> CmdResult<()> {
    crate::feat::traffic_obfuscation_reset_stats().await.stringify_err()
}

/// 检查流量混淆是否正在运行
#[tauri::command]
pub async fn traffic_obfuscation_is_running() -> CmdResult<bool> {
    Ok(crate::feat::traffic_obfuscation_is_running().await)
}
