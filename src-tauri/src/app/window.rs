...
pub async fn quit() {
    let exit_code = prepare_shutdown().await;
    let app_handle = handle::Handle::app_handle();
    app_handle.exit(exit_code);
}

pub async fn prepare_shutdown() -> i32 {
    logging!(debug, Type::System, "启动退出流程");
    handle::Handle::global().set_is_exiting();

    crate::utils::server::shutdown_embedded_server();
...
pub async fn clean_async() -> bool {
    ...
    let core_task = tokio::task::spawn(async {
        ...
        match timeout(stop_timeout, CoreManager::global().stop_core()).await {
            ...
            Err(_) => {
                logging!(warn, Type::Window, "Warning: 停止core超时，继续退出");
                false
            }
        }
    });
