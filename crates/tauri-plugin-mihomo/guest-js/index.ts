...
export async function getDnsMetrics(): Promise<DnsMetrics> {
  return await invoke<DnsMetrics>("plugin:mihomo|get_dns_metrics");
}
...
export async function dnsWarmup(): Promise<void> {
  await invoke<void>("plugin:mihomo|dns_warmup");
}
 ...
 */
export async function disableRules(payload: Record<number, boolean>): Promise<void> {
  await invoke<void>("plugin:mihomo|disable_rules", { payload });
}
...
export async function deleteRule(index: number): Promise<void> {
  await invoke<void>("plugin:mihomo|delete_rule", { index });
}
 ...
 */
export async function createRule(ruleType: string, payload: string, proxy: string, source?: string, subRule?: string, position?: string): Promise<number> {
  return await invoke<number>("plugin:mihomo|create_rule", { ruleType, payload, proxy, source, subRule, position });
}
 ...
 */
export async function getSubRules(): Promise<Record<string, Rule[]>> {
  return await invoke<Record<string, Rule[]>>("plugin:mihomo|get_sub_rules");
}
 ...
 */
export async function deleteSubRuleBySource(name: string, sourcePrefix?: string): Promise<number> {
  return await invoke<number>("plugin:mihomo|delete_sub_rule_by_source", { name, sourcePrefix });
}
