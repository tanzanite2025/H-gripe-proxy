...
impl Mihomo {
    ...
    /// DNS 预解析（预热常用域名缓存）
    pub async fn dns_warmup(&self) -> Result<()> {
    ...
    /// 获取指定 source 的规则（服务端过滤）
    pub async fn get_rules_by_source(&self, source: &str) -> Result<Rules> {
        let path = format!("/rules?source={}", source);
    ...
    /// 禁用或启用规则
    pub async fn disable_rules(&self, payload: &HashMap<i32, bool>) -> Result<()> {
        let client = { self.build_request(Method::PATCH, "/rules/disable")?.json(&payload) };
        let response = self.send_by_protocol(client).await?;
        if !response.status().is_success() {
            let err_msg = response
                .json::<ErrorResponse>()
                .await
                .map_or_else(|e| format!("disable rules failed, {}", e), |err_res| err_res.message);
            ret_failed_resp!("{}", err_msg);
        }
        Ok(())
    }
    ...
    pub async fn delete_rule(&self, index: i32) -> Result<()> {
        let path = format!("/rules/{}", index);
        let client = self.build_request(Method::DELETE, &path)?;
        let response = self.send_by_protocol(client).await?;
        if !response.status().is_success() {
            let err_msg = response
                .json::<ErrorResponse>()
                .await
                .map_or_else(|e| format!("delete rule failed, {}", e), |err_res| err_res.message);
            ret_failed_resp!("{}", err_msg);
        }
        Ok(())
    }
    ...
    pub async fn create_rule(&self, rule_type: &str, payload: &str, proxy: &str, source: Option<&str>, sub_rule: Option<&str>, position: Option<&str>) -> Result<i32> {
        ...
        let body = CreateRuleReq {
            r#type: rule_type.to_string(),
            payload: payload.to_string(),
            proxy: proxy.to_string(),
            source: source.map(|s| s.to_string()),
            sub_rule: sub_rule.map(|s| s.to_string()),
            position: position.map(|s| s.to_string()),
        };
        let client = { self.build_request(Method::POST, "/rules")?.json(&body) };
        let response = self.send_by_protocol(client).await?;
        if !response.status().is_success() {
            let err_msg = response
                .json::<ErrorResponse>()
                .await
                .map_or_else(|e| format!("create rule failed, {}", e), |err_res| err_res.message);
            ret_failed_resp!("{}", err_msg);
        }
        let result: serde_json::Value = response.json().await?;
        let index = result["index"].as_i64().unwrap_or(-1) as i32;
        Ok(index)
    }

    /// Get all sub-rules
    pub async fn get_sub_rules(&self) -> Result<serde_json::Value> {
        let client = self.build_request(Method::GET, "/rules/sub")?;
        let response = self.send_by_protocol(client).await?;
        if !response.status().is_success() {
            let err_msg = response
                .json::<ErrorResponse>()
                .await
                .map_or_else(|e| format!("get sub rules failed, {}", e), |err_res| err_res.message);
            ret_failed_resp!("{}", err_msg);
        }
    ...
    pub async fn delete_sub_rule_by_source(&self, name: &str, source_prefix: Option<&str>) -> Result<i32> {
        let path = format!("/rules/sub/{}", name);
        #[derive(serde::Serialize)]
        #[serde(rename_all = "camelCase")]
        struct DeleteSubRuleReq {
            #[serde(skip_serializing_if = "Option::is_none")]
            source_prefix: Option<String>,
        }
        let body = DeleteSubRuleReq {
            source_prefix: source_prefix.map(|s| s.to_string()),
        };
        ...
        let result: serde_json::Value = response.json().await?;
        let deleted = result["deleted"].as_i64().unwrap_or(0) as i32;
        Ok(deleted)
    }
