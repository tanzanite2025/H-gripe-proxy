...
The next blocker is Mihomo service fallback retirement, not another default-forwarding, packet-capture, fallback-retirement, final-removal, release-packaging, or sidecar-invocation readiness gate. Work through one cohesive service-retirement slice while preserving explicit rollback evidence:

1. Service fallback retirement: consume sidecar invocation retirement, stop defaulting service startup to the Mihomo binary, and keep rollback evidence for operator restore paths.
...
Full protocol replacement and default DNS ownership remain blocked until service fallback retirement proves runtime startup no longer depends on Mihomo packaging or service binary paths.
