...
| Protocol forwarding | Encrypted-protocol Rust preflight in progress | Rust now owns loopback TCP/HTTP forwarding, DIRECT/REJECT policy, bounded remote transport, HTTP CONNECT tunneling, Shadowsocks-style AEAD framing, and Trojan auth framing evidence. Mihomo still owns full encrypted proxy sessions, SOCKS UDP, TUN, and default forwarding. |
...
| Next real batch | `rust-shadowsocks-aead-adapter-execution` | Turn the bounded Shadowsocks AEAD framing evidence into a scoped adapter execution path before VMess/VLESS/Trojan full-session ownership. |
...
| Protocol forwarding | Encrypted-protocol Rust preflight in progress | DIRECT/REJECT, bounded remote transport, HTTP CONNECT, Shadowsocks AEAD framing, and Trojan auth framing evidence are Rust-owned; full encrypted sessions and UDP remain Mihomo fallback. |
...
These blockers allow one useful next PR: `rust-shadowsocks-aead-adapter-execution`. They block full protocol replacement and any claim that packet capture is Rust-owned.
...
Allowed only through the corrected real fast-track sequence above. The current next batch is `rust-shadowsocks-aead-adapter-execution`; keep execution scoped to supported canary evidence and retain unsupported fallback.
