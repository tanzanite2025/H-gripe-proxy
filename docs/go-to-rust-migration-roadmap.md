...
| Protocol forwarding | Unsupported protocol expansion in progress | Rust now owns loopback TCP/HTTP forwarding, DIRECT/REJECT policy, bounded remote transport, HTTP CONNECT tunneling, encrypted framing preflight, scoped Shadowsocks AEAD adapter execution, AEAD canary evidence, multi-chunk encrypted TCP session evidence, bounded SOCKS5 UDP ASSOCIATE datagram forwarding, bounded SOCKS5 username/password negotiation, bounded SOCKS5 TCP CONNECT forwarding, bounded SOCKS5 BIND forwarding, bounded SOCKS5 UDP two-fragment reassembly, bounded VMess/VLESS/Trojan loopback TCP canary sessions, bounded UDP/plugin transport bundle evidence, non-loopback TCP canary evidence, multiplex frame evidence, plugin lifecycle manifest evidence, and plugin process health/crash/restart supervision evidence, non-loopback QUIC-like UDP transcript evidence, QUIC/UDP profile matrix evidence, and bounded default-forwarding hold evidence. Mihomo still owns real remote QUIC peer compatibility, real plugin binary compatibility, system-wide packet capture, and operator-approved production default forwarding cutover. |
| TUN / system proxy | Bounded packet-capture hold evidence in progress | Rust now owns explicit off/system-proxy/TUN route-mode planning, OS system-proxy apply through the Sysopt/sysproxy path, TUN config/restart apply through the existing backend, rollback records, rollback apply, bounded transparent IPv4/TCP packet parsing/execution evidence, repeated platform route rollback hold replay, bounded packet-capture canary parsing, route snapshot/restore-plan evidence, synthetic packet-capture hold evidence, and loopback-only DNS leak telemetry. Mihomo/service still owns system-wide packet capture and transparent forwarding defaults. |
| Mihomo fallback retirement | Bounded fallback-retirement bundle complete | Rust now writes wider execution manifests, emergency rollback checkpoints, default data-plane closeout manifests, unsupported-path fallback continuity evidence, hold telemetry, sidecar source/binary dependency audit evidence, sidecar-independent rollback archive evidence, and selective supported-scope fallback retirement evidence across the bounded DNS/adapter/protocol/UDP/plugin/TUN packet-capture inventory. Unsupported SOCKS non-loopback UDP plus fragment queues/timeouts, operator-approved production DNS cutover/privileged system resolver apply-restore, full GeoIP database loading, real remote QUIC peer compatibility, real plugin binary compatibility, privileged OS route mutation on real interfaces, real TUN packet capture hold, transparent proxy defaults, operator-approved production default forwarding cutover, and full Mihomo binary removal remain fallback-owned until default-path blockers are removed. |
| Next blocker | `default-path-blocker-reduction` | Sidecar-independent rollback, bounded DNS live-resolver/cache/geodata-refresh, read-only route/packet-capture, and protocol non-loopback/multiplex/plugin-manifest blockers and plugin process health/crash/restart supervision, and QUIC/UDP local profile blockers and bounded default-forwarding hold are reduced. Continue only on retained default-path dependencies such as operator-approved production DNS cutover/privileged system resolver apply-restore, real remote QUIC peer compatibility, real plugin binary compatibility, privileged route mutation/real TUN packet-capture hold, operator-approved production default forwarding cutover, or sidecar binary removal after unsupported fallback is empty. |
...
| TUN / system proxy | Bounded packet-capture hold evidence in progress | The TUN/packet-capture hold bundle now supplies repeated rollback, route restoration, packet-capture canary, and DNS leak telemetry evidence. Keep system-wide capture/default routing on Mihomo until the fallback-retirement bundle. |
| Mihomo fallback retirement | Sidecar-independent rollback archive in progress | The accelerated implementation bundles and final review are complete. Rust now archives rollback evidence without invoking the Mihomo sidecar, but sidecar binary removal remains blocked until unsupported runtime fallback is empty and default paths have execution, rollback, hold, and leak evidence. |
