...
| Mihomo fallback retirement | Bounded fallback-retirement bundle complete | Rust now writes wider execution manifests, emergency rollback checkpoints, default data-plane closeout manifests, unsupported-path fallback continuity evidence, hold telemetry, sidecar source/binary dependency audit evidence, sidecar-independent rollback archive evidence, and selective supported-scope fallback retirement evidence across the bounded DNS/adapter/protocol/UDP/plugin/TUN packet-capture inventory. Operator-approved production SOCKS UDP default forwarding cutover and real-profile UDP hold, operator-approved production DNS cutover/privileged system resolver apply-restore, operator-approved production geodata refresh and file availability, real remote encrypted/QUIC peer compatibility, operator-approved real plugin binary compatibility, default-forwarding rollback-surface retirement, TUN/packet-capture rollback-surface retirement, and full Mihomo binary removal remain fallback-owned until default-path blockers are removed. |
| Next blocker | `fallback-retirement-bundle` | Guarded TUN/packet-capture apply now consumes hold, lifecycle, route rollback, and packet leak gates together, writes apply and rollback checkpoint evidence, and demotes Mihomo/service TUN packet-capture fallback to restore-only. The next useful work is fallback retirement closeout, not another packet-capture readiness wrapper. |
...
The next blocker is fallback retirement closeout, not another default-forwarding or packet-capture readiness gate. Work through one cohesive fallback-retirement slice while preserving explicit rollback evidence:

1. Fallback-retirement bundle: unsupported-path fallback continuity, emergency rollback, hold telemetry, and selective Mihomo dependency deprecation/removal only for surfaces with guarded apply evidence.
2. Final closeout bundle: rollback-surface retirement, release blocker audit, and final Mihomo binary removal gate linkage.

Full protocol replacement, default DNS ownership, and Mihomo binary removal remain blocked until fallback-retirement closeout proves rollback-surface retirement is safe.
