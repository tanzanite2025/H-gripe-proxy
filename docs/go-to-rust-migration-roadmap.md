...
The next blocker is runtime service cleanup, not another default-forwarding, packet-capture, fallback-retirement, final-removal, release-packaging, sidecar-invocation, or service-readiness gate. Work through one cohesive cleanup slice while preserving explicit rollback evidence:

1. Runtime service cleanup: remove or isolate remaining service/sidecar stop/log IPC callsites that only exist for rollback cleanup.
2. Final closeout bundle: runtime service callsite retirement, release blocker audit, and final Mihomo binary removal evidence linkage.

Full protocol replacement and default DNS ownership remain blocked until runtime service cleanup proves normal startup no longer depends on Mihomo packaging, sidecar, or service binary paths.
