...
| Next blocker | `privileged-tun-device-lifecycle-blocker` | Sidecar-independent rollback, bounded SOCKS UDP non-loopback fragments/timeouts, bounded encrypted protocol non-loopback defaults, bounded plugin binary compatibility contracts, bounded DNS live-resolver/cache/geodata-refresh, bounded GeoIP database loading, read-only route/packet-capture, bounded TUN lifecycle, read-only route mutation rollback, bounded packet leak hold, protocol non-loopback/multiplex/plugin-manifest blockers, plugin process health/crash/restart supervision, QUIC/UDP local profile blockers, bounded default-forwarding hold, bundled manual default-path removal review, operator-approved default-path cutover manifest consumption, Go/Mihomo final removal gate linkage, guarded retirement execution linkage, post-execution verification linkage, rollback-surface retirement linkage, completion closeout linkage, Rust data-plane hardening preflight linkage, boundary audit linkage, opt-in execution guard linkage, opt-in dry-run linkage, opt-in execution linkage, opt-in execution verification linkage, controlled rollout guard linkage, controlled rollout dry-run linkage, controlled rollout readiness closeout linkage, controlled rollout canary execution linkage, controlled rollout canary verification linkage, supported default promotion chain linkage, expanded default rollout chain linkage, Mihomo fallback retirement readiness linkage, fallback retirement execution linkage, Rust default data-plane closeout linkage, unsupported protocol execution linkage, and route/packet-capture privileged hold linkage are reduced. Route/packet-capture blocker reduction now requires executed unsupported-protocol evidence with closed-out default data-plane confirmation, and TUN packet-capture hold requires that route/packet-capture evidence before running; the next useful work is privileged TUN device lifecycle ownership. |
...
- Do not append one numbered row per canary. Update the owning bundle checklist
  instead.
- The three accelerated implementation bundles, bundled manual default-path removal review, operator-approved default-path cutover manifest path, Go/Mihomo final removal gate linkage, guarded retirement execution linkage, post-execution verification linkage, rollback-surface retirement linkage, completion closeout linkage, Rust data-plane hardening preflight linkage, boundary audit linkage, opt-in execution guard linkage, opt-in dry-run linkage, opt-in execution linkage, opt-in execution verification linkage, controlled rollout guard linkage, controlled rollout dry-run linkage, controlled rollout readiness closeout linkage, controlled rollout canary execution linkage, controlled rollout canary verification linkage, supported default promotion chain linkage, expanded default rollout chain linkage, Mihomo fallback retirement readiness linkage, fallback retirement execution linkage, Rust default data-plane closeout linkage, unsupported protocol execution linkage, and route/packet-capture privileged hold linkage are complete; future work must target privileged TUN device lifecycle ownership rather than more packet-capture readiness wrappers.
