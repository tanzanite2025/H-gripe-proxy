...
Status is current through bounded SOCKS TCP CONNECT execution. The migration has now moved past the earlier gate-only detour and has real bounded Rust data-plane implementations for DNS, adapter policy, loopback forwarding, HTTP CONNECT, encrypted framing, scoped Shadowsocks AEAD execution, AEAD canary evidence, multi-chunk encrypted TCP session evidence, bounded transparent IPv4/TCP packet execution, wider fallback retirement manifest/checkpoint execution, default-scope closeout ownership reconciliation, loopback-only SOCKS5 UDP ASSOCIATE datagram forwarding, deterministic one-domain DNS fake-ip allocation, bounded fallback-filter domain/ipcidr evaluation, bounded nameserver-policy exact/suffix dispatch, bounded SOCKS5 username/password negotiation, and bounded SOCKS5 TCP CONNECT forwarding. The old `rust-data-plane-hardening-*` IPC commands remain safety metadata only; ownership claims below are limited to the explicitly named bounded paths.
...
These blockers allow the next useful implementation PR after `rust-socks-tcp-connect-execution`: `unsupported-protocol-and-packet-capture-implementation`. They block full protocol replacement and any claim that packet capture is Rust-owned.
...
Allowed only through the corrected real fast-track sequence above. The current next batch is `unsupported-protocol-and-packet-capture-implementation` after bounded SOCKS TCP CONNECT execution; keep execution scoped to supported canary evidence and retain unsupported fallback.
