...
| Next blocker | `operator-approved-default-path-cutover` | Sidecar-independent rollback, bounded SOCKS UDP non-loopback fragments/timeouts, bounded encrypted protocol non-loopback defaults, bounded plugin binary compatibility contracts, bounded DNS live-resolver/cache/geodata-refresh, bounded GeoIP database loading, read-only route/packet-capture, bounded TUN lifecycle, read-only route mutation rollback, bounded packet leak hold, protocol non-loopback/multiplex/plugin-manifest blockers, plugin process health/crash/restart supervision, QUIC/UDP local profile blockers, bounded default-forwarding hold, and bundled manual default-path removal review are reduced. The final review now consumes the manual approval artifact to demote approved retained fallback scopes; the next useful work must be the actual operator-approved production cutover path, not more blocker wrappers. |
...
- Do not append one numbered row per canary. Update the owning bundle checklist
  instead.
- The three accelerated implementation bundles and bundled manual default-path removal review are complete; future work must target the operator-approved production cutover path or sidecar binary removal after unsupported fallback is empty.
