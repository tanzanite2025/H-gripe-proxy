...
| Next blocker | `rust-data-plane-hardening-opt-in-dry-run` | Sidecar-independent rollback, bounded SOCKS UDP non-loopback fragments/timeouts, bounded encrypted protocol non-loopback defaults, bounded plugin binary compatibility contracts, bounded DNS live-resolver/cache/geodata-refresh, bounded GeoIP database loading, read-only route/packet-capture, bounded TUN lifecycle, read-only route mutation rollback, bounded packet leak hold, protocol non-loopback/multiplex/plugin-manifest blockers, plugin process health/crash/restart supervision, QUIC/UDP local profile blockers, bounded default-forwarding hold, bundled manual default-path removal review, operator-approved default-path cutover manifest consumption, Go/Mihomo final removal gate linkage, guarded retirement execution linkage, post-execution verification linkage, rollback-surface retirement linkage, completion closeout linkage, Rust data-plane hardening preflight linkage, boundary audit linkage, and opt-in execution guard linkage are reduced. The final removal gate, execution report, post-execution verification, rollback-surface retirement, completion closeout, hardening preflight, boundary audit, and opt-in execution guard now consume committed operator cutover evidence before advancing Rust-only data-plane work; the next useful work is opt-in dry-run tied to execution guard evidence. |
...
- Do not append one numbered row per canary. Update the owning bundle checklist
  instead.
- The three accelerated implementation bundles, bundled manual default-path removal review, operator-approved default-path cutover manifest path, Go/Mihomo final removal gate linkage, guarded retirement execution linkage, post-execution verification linkage, rollback-surface retirement linkage, completion closeout linkage, Rust data-plane hardening preflight linkage, boundary audit linkage, and opt-in execution guard linkage are complete; future work must target opt-in dry-run tied to execution guard evidence rather than more readiness wrappers.
