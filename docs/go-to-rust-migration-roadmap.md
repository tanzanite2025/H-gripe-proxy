...
| Next blocker | `mihomo-sidecar-binary-removal` | Sidecar-independent rollback, bounded SOCKS UDP non-loopback fragments/timeouts, bounded encrypted protocol non-loopback defaults, bounded plugin binary compatibility contracts, bounded DNS live-resolver/cache/geodata-refresh, bounded GeoIP database loading, read-only route/packet-capture, bounded TUN lifecycle, read-only route mutation rollback, bounded packet leak hold, protocol non-loopback/multiplex/plugin-manifest blockers, plugin process health/crash/restart supervision, QUIC/UDP local profile blockers, bounded default-forwarding hold, bundled manual default-path removal review, and operator-approved default-path cutover manifest consumption are reduced. The final review now consumes a committed operator cutover manifest to demote approved retained fallback scopes; the next useful work is sidecar binary removal only after committed cutover and rollback coverage are present. |
...
- Do not append one numbered row per canary. Update the owning bundle checklist
  instead.
- The three accelerated implementation bundles, bundled manual default-path removal review, and operator-approved default-path cutover manifest path are complete; future work must target sidecar binary removal after unsupported fallback is empty and rollback coverage is committed.
