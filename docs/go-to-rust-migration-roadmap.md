...
The next blocker is the TUN/packet-capture hold bundle, not another default-forwarding readiness gate or one-file/one-parser PR. Work through one cohesive real-interface hold/rollback slice while preserving explicit Mihomo fallback:
...
Full protocol replacement, packet-capture ownership, default DNS ownership, and Mihomo binary removal remain blocked until TUN/packet-capture hold and remaining real-interface rollback evidence pass.
