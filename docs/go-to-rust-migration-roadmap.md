...
The next blocker is sidecar invocation retirement, not another default-forwarding, packet-capture, fallback-retirement, final-removal, or release-packaging readiness gate. Work through one cohesive sidecar-invocation slice while preserving explicit rollback evidence:

1. Sidecar invocation retirement: consume release closeout, stop defaulting runtime startup to Mihomo sidecar invocation, and keep rollback evidence for service/fallback restore paths.
2. Final closeout bundle: runtime sidecar callsite retirement, release blocker audit, and final Mihomo binary removal evidence linkage.

Full protocol replacement and default DNS ownership remain blocked until sidecar invocation retirement proves runtime startup no longer depends on Mihomo packaging.
