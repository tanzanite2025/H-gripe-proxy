...
| Final closeout | `final-go-to-rust-closeout-bundle` in progress | Mihomo plugin API retirement now removes direct Go-plugin mutation/recovery paths for base-config patching, live config reload, geo/core/UI upgrades, DNS warmup, TLS rotation, proxy selection/provider healthcheck/update, runtime rule mutation, close-all-connections, egress rebind, residential verification rule/proxy mutation, security policy rule mutation, and tray close-all actions. Runtime activation now enters the Rust restart boundary instead of the retired live-reload/no-restart path; remaining plugin references are type/compatibility inventory or unsupported-path fallback evidence. |
...
The current useful migration PR shape is a single `final-go-to-rust-closeout-bundle`.
It collapses remaining blockers into one reviewable PR:

- Resolve the last live config reload retirement path in `core::manager::config`
  by deleting the retired no-restart/live-reload path and routing activation
  through the Rust restart boundary.
...
- Maintain the final release-blocker table below: supported paths owned by Rust,
  retained Mihomo-owned unsupported paths, rollback command, and evidence
  file/source.
...
| Surface | Rust-owned supported path | Retained Mihomo-owned unsupported path | Rollback / evidence source |
| --- | --- | --- | --- |
| Runtime config activation | `runtime_lifecycle::update_runtime_config_with_restart_boundary` validates generated runtime config and applies it through `CoreManager::restart_core`; the retired live-reload/no-restart branch is deleted. | No app-layer live reload through the Go/Mihomo plugin API remains. Unsupported production startup remains blocked until Rust runtime startup is ready. | Runtime draft discard on failure plus lifecycle audit in `src-tauri/src/core/runtime_lifecycle.rs`; apply boundary in `src-tauri/src/core/manager/config.rs`. |
| DNS / adapter / protocol / UDP / plugin / TUN default paths | Rust-owned bounded evidence and opt-in apply paths remain the supported migration inventory. | Mihomo still owns unsupported production defaults that lack operator-approved cutover or hold history. | Kernel evidence and rollback records under `src-tauri/src/core/kernel_runtime/`; runtime snapshot lifecycle records. |
| Sidecar and release packaging | App startup and release metadata no longer launch or package a default Mihomo sidecar for supported paths. | Emergency rollback/fallback evidence can still name Mihomo for unsupported retained paths. | Release/fallback evidence in `src-tauri/src/core/kernel_runtime/go_retirement.rs` and `rust_runtime_cutover.rs`. |
| Frontend/app command surface | Frontend-facing control mutations are routed through Rust runtime lifecycle, projection, snapshot, and audit surfaces. | `tauri_plugin_mihomo` model imports can remain as compatibility DTOs until consumers are replaced by Rust-native DTOs. | Command and projection callsites under `src-tauri/src/cmd/` and `src-tauri/src/core/app_runtime/`. |
...
1. `final-go-to-rust-closeout-bundle`: remove the remaining live-reload/no-restart activation path, audit stale Go/Mihomo ownership references, and update release/fallback evidence in one PR.
...
Full protocol replacement and default DNS ownership remain blocked until Rust-owned runtime startup, observation, and command-facing control mutations can run supported defaults without the Go plugin API.
