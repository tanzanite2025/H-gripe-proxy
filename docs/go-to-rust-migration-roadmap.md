...
| Next blocker | `rust-data-plane-hardening-preflight` | Sidecar-independent rollback, bounded SOCKS UDP non-loopback fragments/timeouts, bounded encrypted protocol non-loopback defaults, bounded plugin binary compatibility contracts, bounded DNS live-resolver/cache/geodata-refresh, bounded GeoIP database loading, read-only route/packet-capture, bounded TUN lifecycle, read-only route mutation rollback, bounded packet leak hold, protocol non-loopback/multiplex/plugin-manifest blockers, plugin process health/crash/restart supervision, QUIC/UDP local profile blockers, bounded default-forwarding hold, bundled manual default-path removal review, operator-approved default-path cutover manifest consumption, Go/Mihomo final removal gate linkage, guarded retirement execution linkage, post-execution verification linkage, rollback-surface retirement linkage, and completion closeout linkage are reduced. The final removal gate, execution report, post-execution verification, rollback-surface retirement, and completion closeout now consume committed operator cutover evidence before declaring Go/Mihomo retirement complete; the next useful work is Rust data-plane hardening preflight after retirement closeout. |
...
- Do not append one numbered row per canary. Update the owning bundle checklist
  instead.
- The three accelerated implementation bundles, bundled manual default-path removal review, operator-approved default-path cutover manifest path, Go/Mihomo final removal gate linkage, guarded retirement execution linkage, post-execution verification linkage, rollback-surface retirement linkage, and completion closeout linkage are complete; future work must target Rust data-plane hardening preflight after retirement closeout rather than more readiness wrappers.
