...
### Batch V：Default DNS runtime expanded stability gate（本批次，保持 active 的显式决策）

Batch U 已能判断 expanded execution 后是否观测通过。本批次继续收束成一个显式 stability gate：只决定“当前 session 是否可继续保持 expanded active reload”，不把它升级为长期默认，也不自动 rollback。

```text
feat(dns-runtime): add expanded stability gate
```

建议边界：

- Stability gate 必须消费 Batch U expanded post-execution verification。
- 必须要求用户显式触发；不能在页面加载、verification 或 rollback drill 后自动执行。
- 只有 expanded post-execution verified、failure audit 不 required、expanded rollback drill ready、active state 为 `expandedActiveProfileReloaded` 时才允许 keep-active。
- Report 必须明确 `keepActiveAllowed`、`rollbackRecommended`、`promotionAllowed=false`、`autoRollout=false`、`autoRollback=false`。

不包含：

- 不进行长期默认推广。
- 不自动回滚；rollback 仍必须由 `dns_default_runtime_expanded_rollback` 显式触发。
- 不扩大 rollout 到 TUN、transparent proxy、adapter outbound/inbound 或 protocol runtime。
...
### Batch X：Default DNS runtime expanded repeated reverify（本批次，重复显式验证审计）

Batch W 已定义时间窗策略。本批次把每次用户显式 reverify 固化为 Rust-owned audit record，支持在同一 session 内重复记录 hold-window evaluation，而不是只看一次 UI report。

```text
feat(dns-runtime): add expanded reverify audit
```

建议边界：

- Reverify 必须消费 Batch W hold policy。
- 每次 reverify 都必须由用户显式触发，并持久化独立 audit record。
- Record 必须包含 hold status、stability status、post-execution status、active age、next verification / hold expiry、rollback recommendation。
- 当 hold policy 为 `rollbackRecommended` 时仍可持久化 record，但只建议显式 rollback，不自动调用 rollback。
- Report 必须明确 `autoRollout=false`、`autoRollback=false`、`mutatesRuntime=false`、`reloadMihomo=false`。

不包含：

- 不在后台定时 reverify。
- 不自动 rollback。
- 不汇总长期 history 或做 rollout promotion。
- 不触碰 TUN、transparent proxy、adapter outbound/inbound 或 protocol runtime。

已落地范围：

- 新增 `dns_default_runtime_expanded_reverify` command。
- 每次显式 reverify 持久化 `expanded-reverify/<event>/reverify.yaml`。
- Report 输出 reverify record、persisted path、hold policy、rollback recommendation 与 next verification 状态。
- DNS runtime stats UI 新增 Expanded reverify 显式按钮与结果展示。

### Batch Y：Default DNS runtime expanded reverify history / trend / closeout（本批次，合并多步）

按“几步合并”的粒度，本批次不再只做一个小按钮，而是把 reverify history、trend threshold、session closeout 建议合在一个功能块里。目标是让 Batch X 的多次显式 reverify record 能形成一个 Rust-owned 汇总判断。

```text
feat(dns-runtime): add expanded reverify history closeout
```

合并范围：

- History：读取 `expanded-reverify/<event>/reverify.yaml` 下所有显式 reverify audit record。
- Trend：统计 keep-active、rollback recommended、blocked、next verification required、stable streak。
- Threshold：连续稳定 reverify 数达到阈值时输出 `closeoutReady=true`。
- Closeout：只关闭当前 session 的 observation window，不推广为长期默认。

建议边界：

- History summary 只读取已持久化 record，不触发新的 reverify。
- 发现 rollback recommended trend 时只建议显式 rollback。
- `promotionAllowed=false`、`autoRollout=false`、`autoRollback=false`、`mutatesRuntime=false`、`reloadMihomo=false` 必须保持。
- Empty history 是可解释状态，不应伪造成功。

不包含：

- 不自动创建 reverify record。
- 不自动 rollback。
- 不推广为长期默认。
- 不触碰 TUN、transparent proxy、adapter outbound/inbound 或 protocol runtime。
...
- 新增 `dns_default_runtime_expanded_reverify_history` command。
- Report 输出 record list、latest record、status counts、stable streak、required threshold、rollback trend 与 closeout recommendation。
- DNS runtime stats UI 新增 Reverify history 显式按钮与汇总面板。

### Batch Z：Default DNS runtime expanded lifecycle closeout / handoff（本批次，合并交接决策）

本批次继续合并粒度：不把 closeout 与下一控制面交接拆成单独微 PR，而是在同一功能块里把 reverify history、active state、observation closeout、handoff recommendation 串起来。

```text
feat(dns-runtime): add expanded lifecycle closeout
```

合并范围：

- Closeout：消费 Batch Y reverify history summary。
- Active state：确认当前 active state 仍是 `expandedActiveProfileReloaded`。
- Handoff：当 stable threshold 满足且 active state 匹配时输出 `handoffReady=true`。
- Next step：给出下一控制面工作建议，但仍不进入真实数据面。

建议边界：

- Lifecycle closeout 只读 history 与 active state，不写入新状态。
- Rollback trend 只输出 explicit rollback recommendation。
- Handoff 是 control-plane handoff，不是 permanent DNS runtime promotion。
- `promotionAllowed=false`、`autoRollout=false`、`autoRollback=false`、`mutatesRuntime=false`、`reloadMihomo=false` 必须保持。

不包含：

- 不自动 close / delete active marker。
- 不自动 rollback。
- 不推广为长期默认。
- 不触碰 TUN、transparent proxy、adapter outbound/inbound 或 protocol runtime。
...
### Batch AC：App runtime control-plane completion bundle（本批次，继续加大）

在 Batch AB 接收 DNS handoff 之后，本批次继续把 app-runtime 主线后续多步合并为一个 control-plane completion bundle：一次命令完成 DNS handoff intake、应用规划诊断、Mihomo projection artifact 持久化、staged activation preflight、聚合诊断与 UI 面板更新。

```text
complete_app_runtime_control_plane(appId)
  -> accept_app_runtime_dns_handoff()
  -> diagnose_app_runtime(appId)
  -> build + persist AppRuntimeProjectionArtifact
  -> preflight_app_runtime_projection_activation(artifactId, checksum)
  -> ready | degraded | blocked
```

合并范围：

- Command：新增 `complete_app_runtime_control_plane`，串联 DNS handoff、projection artifact 与 staged preflight。
- Report：新增 `AppRuntimeControlPlaneCompletionReport`，输出 `readyForStagedActivation`、artifact path、preflight status、next step 与 blockers。
- UI：高级页新增“完成 control-plane”按钮、聚合诊断状态与 completion 结果面板。
- Tests：覆盖 accepted DNS handoff + artifact + staged preflight 的 ready 汇总。

强边界：

- 不执行 `activate_app_runtime_projection_artifact`。
- 不执行 `apply_app_runtime_projection_artifact_to_runtime`。
- 不 reload Mihomo，不写 active runtime apply marker。
...
### Batch AH：Runtime apply verification decision checkpoint（本批次，post-apply 反查决策链）

Batch AG 已要求 runtime apply command 在写 runtime candidate 前携带 persisted allow decision。本批次继续把 post-apply observed verification 接到同一事实链：验证报告必须反查 runtime apply audit 中记录的 decision id，并确认该 decision 仍匹配 artifact / checksum / boundary manifest。

```text
verify_app_runtime_projection_runtime_apply(artifactId)
  -> latest runtime apply audit
  -> read runtime-apply-decisions/<audit.decisionId>/decision.yaml
  -> require allowRuntimeCandidate + accepted + artifact/checksum/audit match
  -> expose runtimeApplyDecisionVerified in verification report/UI
```

合并范围：

- Verification：新增 `runtime_apply_decision_checkpoint` diagnostic check，缺少 decision id、读取失败、defer / rollback / checksum mismatch 都会让 observed verification fail。
- Report：verification report 暴露 `runtimeApplyDecisionId` / `runtimeApplyDecisionBoundaryManifestId` / `runtimeApplyDecisionVerified`。
- UI：runtime verification 面板展示 decision checkpoint 是否 verified。
- Tests：覆盖匹配 allow decision 通过、缺失或非 allow decision 失败。

强边界：

- 只读验证；不触发 runtime apply，不 reload Mihomo。
- 不放宽 Batch AG 的 command guard。
- 不进入 Phase 8 / adapter / tunnel / protocol runtime。

不包含：

- 不新增自动 rollout / rollback。
- 不扩大默认 DNS runtime 或协议 runtime 执行范围。

### Batch AI：Runtime apply verification closeout checkpoint（本批次，验证后收口）

Batch AH 已让 post-apply observed verification 反查 persisted allow decision。本批次继续把验证结果收口成 Rust-owned closeout record：只有 observed verification 非 blocked、runtime apply audit 存在、且 decision checkpoint verified 时，才能生成 complete closeout；closeout 本身仍是只读边界，不代表继续 rollout / reload / Phase 8。

```text
closeout_app_runtime_projection_runtime_apply_verification(artifactId)
  -> verify_app_runtime_projection_runtime_apply(artifactId)
  -> require audit + runtimeApplyDecisionVerified
  -> persist app-runtime/runtime-apply-verification-closeouts/<closeout>/closeout.yaml
  -> keep runtimeApplyAllowed=false / phase8Allowed=false / reloadMihomo=false
```

合并范围：

- Command：新增 `closeout_app_runtime_projection_runtime_apply_verification`，消费 observed verification report 并持久化 closeout record。
- Closeout：记录 artifact/checksum/audit/decision/boundary manifest linkage 与下一步 hold 状态。
- UI：runtime verification 面板新增 closeout action，并展示 closeout status / next step / safety flags。
- Tests：覆盖 verified decision checkpoint 可 complete，未 verified decision checkpoint 阻止 closeout。

强边界：

- Closeout 不触发 runtime apply，不 reload Mihomo。
- `runtimeApplyAllowed=false`、`phase8Allowed=false`、`promotionAllowed=false`。
