...
Sidecar-independent rollback archive and DNS default-path blocker reduction now reduce final-review blockers by adding Rust-owned rollback archives, bounded live resolver evidence, persistent cache migration evidence, and geodata refresh ownership evidence. Keep execution scoped to supported canary evidence and retain unsupported fallback until a specific default-path blocker is removed with new proof.
