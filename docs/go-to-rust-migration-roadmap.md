...
| Next blocker | `mihomo-plugin-api-retirement` | Runtime service cleanup now removes Sidecar mode state, child sidecar process tracking, service start/stop/log IPC entrypoints, sidecar log writer plumbing, Mihomo IPC restart recovery, and the `clash_core` default config surface. The next useful work is retiring direct `tauri_plugin_mihomo` API callsites, not another runtime cleanup wrapper. |
...
The next blocker is Mihomo plugin API retirement, not another default-forwarding, packet-capture, fallback-retirement, final-removal, release-packaging, sidecar-invocation, service-readiness, or runtime-cleanup gate. Work through one cohesive plugin-API retirement slice while preserving explicit rollback evidence:

1. Mihomo plugin API retirement: remove direct `tauri_plugin_mihomo` recovery and mutation callsites that still reach the Go runtime API.
2. Final closeout bundle: plugin API callsite retirement, release blocker audit, and final Mihomo binary removal evidence linkage.

Full protocol replacement and default DNS ownership remain blocked until Mihomo plugin API retirement proves normal runtime calls no longer depend on the Go plugin API.
