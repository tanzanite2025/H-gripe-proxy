...
The next blocker is the final Mihomo binary removal gate, not another default-forwarding, packet-capture, or fallback-retirement readiness gate. Work through one cohesive final-removal slice while preserving explicit rollback evidence:

1. Final Mihomo binary removal gate: consume fallback-retirement closeout, sidecar-independent rollback, release blocker audit, and explicit operator approval before any binary deletion.
...
Full protocol replacement, default DNS ownership, and Mihomo binary removal remain blocked until the final-removal gate proves rollback-surface retirement and release safety.
