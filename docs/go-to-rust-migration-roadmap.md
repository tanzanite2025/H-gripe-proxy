...
The next blocker is guarded production default-forwarding apply, not another readiness gate or one-file/one-parser PR. Work through one cohesive apply/verification slice while preserving explicit Mihomo fallback:

1. Guarded production default-forwarding apply: consume the committed approval manifest, mutate only the approved TCP/SOCKS UDP/encrypted/QUIC/plugin default surfaces, and keep one-switch Mihomo rollback.
2. Post-approval hold verification: run real-profile hold evidence after apply, verify leak/health/rollback history, and only then consider selective Mihomo dependency deprecation/removal.

Full protocol replacement, packet-capture ownership, default DNS ownership, and Mihomo binary removal remain blocked until guarded apply has real Rust execution evidence, rollback history, and post-canary hold proof.
