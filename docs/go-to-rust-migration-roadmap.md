...
| Next blocker | `default-path-blocker-reduction` | The sidecar-independent rollback blocker is reduced by Rust-owned rollback archives. Continue only on retained default-path dependencies such as live DNS replacement, broad non-loopback protocol forwarding, system-wide packet capture/route install, plugin lifecycle, or sidecar binary removal after unsupported fallback is empty. |
...
Sidecar-independent rollback archive now reduces the final-review rollback blocker by copying rollback/evidence YAML into a Rust-owned archive with checksums and a restore plan that never invokes the Mihomo sidecar. Keep execution scoped to supported canary evidence and retain unsupported fallback until a specific default-path blocker is removed with new proof.
