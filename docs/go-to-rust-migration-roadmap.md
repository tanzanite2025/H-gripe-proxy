...
| Next blocker | `manual-default-path-removal-review` | Sidecar-independent rollback, bounded SOCKS UDP non-loopback fragments/timeouts, bounded encrypted protocol non-loopback defaults, bounded plugin binary compatibility contracts, bounded DNS live-resolver/cache/geodata-refresh, bounded GeoIP database loading, read-only route/packet-capture, bounded TUN lifecycle, read-only route mutation rollback, bounded packet leak hold, protocol non-loopback/multiplex/plugin-manifest blockers, plugin process health/crash/restart supervision, QUIC/UDP local profile blockers, and bounded default-forwarding hold are reduced. The next useful step is a bundled manual default-path removal review that accepts or rejects DNS, protocol forwarding, route/packet-capture, and sidecar removal surfaces together instead of adding another one-blocker PR. Runtime mutation still requires operator-approved production cutover. |
...
- Do not append one numbered row per canary. Update the owning bundle checklist
  instead.
- The three accelerated implementation bundles are complete; future work must target a still-retained default-path dependency or final-review blocker, preferably through the bundled manual default-path removal review rather than one blocker per PR.
