...
| Protocol forwarding | Unsupported protocol expansion in progress | Rust now owns loopback TCP/HTTP forwarding, DIRECT/REJECT policy, bounded remote transport, HTTP CONNECT tunneling, encrypted framing preflight, scoped Shadowsocks AEAD adapter execution, AEAD canary evidence, multi-chunk encrypted TCP session evidence, bounded SOCKS5 UDP ASSOCIATE datagram forwarding, bounded SOCKS5 username/password negotiation, bounded SOCKS5 TCP CONNECT forwarding, bounded SOCKS5 BIND forwarding, bounded SOCKS5 UDP two-fragment reassembly, bounded SOCKS UDP fragment queue timeout/reap evidence, bounded VMess/VLESS/Trojan loopback TCP canary sessions, bounded VMess/VLESS/Trojan non-loopback local TCP canary sessions, bounded UDP/plugin transport bundle evidence, non-loopback TCP canary evidence, multiplex frame evidence, plugin lifecycle manifest evidence, and plugin process health/crash/restart supervision evidence, plugin binary startup/stdout/health/crash compatibility contract evidence, non-loopback QUIC-like UDP transcript evidence, QUIC/UDP profile matrix evidence, and bounded default-forwarding hold evidence. Mihomo still owns real remote encrypted/QUIC peer compatibility, operator-approved real plugin binary compatibility, system-wide packet capture, and operator-approved production default forwarding cutover. |
| TUN / system proxy | Bounded packet-capture hold evidence in progress | Rust now owns explicit off/system-proxy/TUN route-mode planning, OS system-proxy apply through the Sysopt/sysproxy path, TUN config/restart apply through the existing backend, rollback records, rollback apply, bounded transparent IPv4/TCP packet parsing/execution evidence, repeated platform route rollback hold replay, bounded packet-capture canary parsing, route snapshot/restore-plan evidence, synthetic packet-capture hold evidence, loopback-only DNS leak telemetry, bounded TUN lifecycle state-machine evidence, TUN rollback ordering evidence, read-only route mutation replay evidence, platform route apply/rollback plan evidence, bounded packet leak hold evidence, and synthetic external-interface leak observation evidence. Mihomo/service still owns system-wide packet capture and transparent forwarding defaults. |
| Mihomo fallback retirement | Bounded fallback-retirement bundle complete | Rust now writes wider execution manifests, emergency rollback checkpoints, default data-plane closeout manifests, unsupported-path fallback continuity evidence, hold telemetry, sidecar source/binary dependency audit evidence, sidecar-independent rollback archive evidence, and selective supported-scope fallback retirement evidence across the bounded DNS/adapter/protocol/UDP/plugin/TUN packet-capture inventory. Operator-approved production SOCKS UDP default forwarding cutover and real-profile UDP hold, operator-approved production DNS cutover/privileged system resolver apply-restore, operator-approved production geodata refresh and file availability, real remote encrypted/QUIC peer compatibility, operator-approved real plugin binary compatibility, privileged TUN device create/destroy, operator-approved privileged route mutation cutover on real interfaces, operator-approved production packet leak hold on real interfaces, transparent proxy defaults, operator-approved production default forwarding cutover, and full Mihomo binary removal remain fallback-owned until default-path blockers are removed. |
| Next blocker | `rust-data-plane-hardening-mihomo-fallback-retirement-execution` | Sidecar-independent rollback, bounded SOCKS UDP non-loopback fragments/timeouts, bounded encrypted protocol non-loopback defaults, bounded plugin binary compatibility contracts, bounded DNS live-resolver/cache/geodata-refresh, bounded GeoIP database loading, read-only route/packet-capture, bounded TUN lifecycle, read-only route mutation rollback, bounded packet leak hold, protocol non-loopback/multiplex/plugin-manifest blockers, plugin process health/crash/restart supervision, QUIC/UDP local profile blockers, bounded default-forwarding hold, bundled manual default-path removal review, operator-approved default-path cutover manifest consumption, Go/Mihomo final removal gate linkage, guarded retirement execution linkage, post-execution verification linkage, rollback-surface retirement linkage, completion closeout linkage, Rust data-plane hardening preflight linkage, boundary audit linkage, opt-in execution guard linkage, opt-in dry-run linkage, opt-in execution linkage, opt-in execution verification linkage, controlled rollout guard linkage, controlled rollout dry-run linkage, controlled rollout readiness closeout linkage, controlled rollout canary execution linkage, controlled rollout canary verification linkage, supported default promotion chain linkage, expanded default rollout chain linkage, and Mihomo fallback retirement readiness linkage are reduced. The final removal gate, execution report, post-execution verification, rollback-surface retirement, completion closeout, hardening preflight, boundary audit, opt-in execution guard, opt-in dry-run, opt-in execution, opt-in execution verification, controlled rollout guard, controlled rollout dry-run, readiness closeout, canary execution, canary verification, supported default promotion chain, expanded default rollout chain, and Mihomo fallback retirement readiness now consume committed operator cutover evidence before advancing Rust-only data-plane work; the next useful work is Mihomo fallback retirement execution tied to readiness evidence. |
...
- Do not append one numbered row per canary. Update the owning bundle checklist
  instead.
- The three accelerated implementation bundles, bundled manual default-path removal review, operator-approved default-path cutover manifest path, Go/Mihomo final removal gate linkage, guarded retirement execution linkage, post-execution verification linkage, rollback-surface retirement linkage, completion closeout linkage, Rust data-plane hardening preflight linkage, boundary audit linkage, opt-in execution guard linkage, opt-in dry-run linkage, opt-in execution linkage, opt-in execution verification linkage, controlled rollout guard linkage, controlled rollout dry-run linkage, controlled rollout readiness closeout linkage, controlled rollout canary execution linkage, controlled rollout canary verification linkage, supported default promotion chain linkage, expanded default rollout chain linkage, and Mihomo fallback retirement readiness linkage are complete; future work must target Mihomo fallback retirement execution tied to readiness evidence rather than more readiness wrappers.
...
The next blocker is not another readiness gate and should not be another one-file/one-parser PR. Work through the bundled sequence above, where each PR retires a coherent Go-owned slice while preserving explicit Mihomo fallback:

1. TUN/packet-capture hold bundle: repeated Windows/macOS/Linux route restoration, TUN/system-proxy rollback drills, transparent-routing default boundaries, packet-capture canaries, DNS leak checks, and health telemetry.
2. Fallback-retirement bundle: unsupported-path fallback continuity, emergency rollback, hold telemetry, and only then selective Mihomo dependency deprecation/removal.

Full protocol replacement, packet-capture ownership, default DNS ownership, and Mihomo binary removal remain blocked until the corresponding bundle has real Rust execution evidence, rollback history, and post-canary hold proof.
