...
>
> 最近更新：**gost-relay 出站数据面已补**——经 go-gost relay 协议 v1 转发：每条连接发一个 `CONNECT` 请求（`VER(0x01)|CMD(0x01)|FEALEN(2)|FEATURES`），特征列表 TLV 编码 `UserAuth`(可选 `username`/`password`)、`Addr`(目标 atyp/addr/port)、`Network`(TCP)，读 4 字节响应头校验 `version==1 && status==0` 后透传；`tls: true` 复用共享 TLS 层（SNI/`skip-cert-verify`/`fingerprint`/`client-fingerprint`），`forward: true` 发空目标地址让 relay 路由到其预设上游；仅 TCP（UDP 关联拒绝），`mux`(smux) 与 mTLS 客户端证书在 config 阶段显式拒绝；线格式逐字段对齐 mihomo `transport/gost`，真实 fake relay server 互通测试见 `tests/gost_relay_outbound.rs`（明文 / 鉴权 / TLS / 错误凭据拒绝）。**hysteria(v1) 出站数据面已补**——Hysteria v1（原版，区别于 hysteria2）经单条 TLS 加密 QUIC 连接（默认 ALPN `hysteria`、协议版本 3）转发：控制流写版本 + `ClientHello{rate,auth}` 鉴权、读 `ServerHello`，每个 TCP 目标开新双向流写 `ClientRequest` 读 `ServerResponse` 后透传；复用现有 `transport::quic`（QUIC/TLS）+ 端口跳跃，新增 v1 专用 **XPlus 包混淆**（`obfs: <key>`，`SHA256(key‖salt16)` XOR，区别于 v2 Salamander 的 BLAKE2b），把 QUIC obfs socket 泛化成 `PacketObfs` 枚举复用；鉴权 `auth-str`/`auth`(base64)，速率 `up`/`down`(+`up-speed`/`down-speed`) 按上游 `stringToBps` 解析；仅 TCP（无 UDP relay），`faketcp`/`wechat-video` 包底层拒绝；真实 quinn v1 server 互通测试见 `tests/hysteria_outbound.rs`（鉴权 + TCP echo + 独立 XPlus codec 逐字节兼容 + 端口跳跃）。**ssh 出站数据面已补**——经 SSH `direct-tcpip` 通道（RFC 4254 §7.2，等价 OpenSSH `ssh -L`）转发；SSH 传输/密钥交换/认证交给 vetted `russh`（纯 Rust `ring` 后端，无 aws-lc-rs / C 工具链），内核自管认证方式选择 + host-key pinning + 开通道 + 流交接；支持 `password` 与 `private-key`(+`private-key-passphrase`) 认证、`host-key` 固定（authorized_keys 文本，置空则不校验）、`host-key-algorithms` 限制；仅 TCP（direct-tcpip 无 UDP relay）；真实 russh server 互通测试见 `tests/ssh_outbound.rs`（密码认证 / 公钥认证 / host-key 不匹配拒绝）。**v2ray-plugin 非 websocket / mux 已补**——`v2ray-http-upgrade`（复用 httpupgrade transport）、`mux: true`（mux.cool 单子连接帧封装）、`mode: quic`（标准 QUIC + 强制 TLS + 按 server 连接池复用，ALPN 默认 `[h2,http/1.1]`），互通测试见 `tests/shadowsocks_plugin.rs`；SS SIP003 plugin 全部完成（simple-obfs http/伪 TLS、v2ray-plugin ws±tls，PR #449/#450）；**ECH（Encrypted Client Hello）已接线**——内置 RFC 9180 HPKE provider（DHKEM-X25519-HKDF-SHA256 + AES-128/256-GCM、ChaCha20Poly1305），`ech-opts` 现已驱动真实握手。
...
| hysteria（v1） | ✅ | ❌ | Hysteria **v1**（原版，区别于 hysteria2）：单条 TLS 加密 QUIC 连接（默认 ALPN `hysteria`，协议版本 3）。握手后开一条控制流写 1 字节版本 + `ClientHello{rate, auth}`，读 `ServerHello{ok,...}`（鉴权失败即拒绝），控制流保持打开（关闭会被服务端视作 session 结束）；每个 TCP 目标开一条新双向流写 `ClientRequest{udp:false, host, port}`、读 `ServerResponse{ok,...}` 后直接透传。结构体用与上游 `apernet/hysteria` 一致的定长大端 + `u16` 长度前缀布局。鉴权 `auth-str`(明文) / `auth`(base64)；`up`/`down`（或 `up-speed`/`down-speed` Mbps）按上游 `stringToBps` 解析为 `ClientHello` 的建议速率（服务端 Brutal 用 `recv_bps`，本地发送侧用 quinn BBR，纯本地选择不影响互通）。**XPlus 包混淆**（`obfs: <key>`，`salt(16)‖payload^SHA256(key‖salt)`，v1 专用、区别于 v2 的 Salamander）+ **端口跳跃**（`ports`）复用 QUIC 之下的 `transport::quic_obfs`（`PacketObfs` 枚举）。仅 TCP（无 UDP relay）；`faketcp`/`wechat-video` 包底层在 config 阶段拒绝。真实 quinn v1 server 互通测试见 `tests/hysteria_outbound.rs`（控制流鉴权 + TCP relay echo；独立 SHA-256 XPlus codec 验证逐字节混淆兼容 + 端口跳跃） |
| ssh | ✅ | ❌ | SSH `direct-tcpip` 通道（RFC 4254 §7.2，等价 `ssh -L`）：SSH 传输/KEX/认证交给 vetted `russh`（纯 Rust `ring` 后端，无 aws-lc-rs / C 工具链），内核自管认证方式选择 + host-key pinning + 开通道 + 流交接；`username` + `password` 或 `private-key`(+`private-key-passphrase`，支持 OpenSSH/PKCS#8/PKCS#5/PuTTY) 认证；`host-key`（authorized_keys 文本，置空则不校验）固定 + `host-key-algorithms` 限制；server 保留为域名按需解析；无 UDP relay（direct-tcpip 仅 TCP）；真实 russh server 互通测试见 `tests/ssh_outbound.rs`（密码 / 公钥 / host-key 不匹配拒绝） |
...
| gost-relay | ✅ | ❌ | go-gost relay 协议 **v1**：每条连接发一个 `CONNECT` 请求 `VER(0x01)|CMD(0x01)|FEALEN(2)|FEATURES`，特征列表 TLV（`TYPE(1)|LEN(2)|DATA`）携带 `UserAuth(0x01)`（`ULEN|UNAME|PLEN|PASSWD`，仅在 `username`/`password` 任一非空时发）、`Addr(0x02)`（`ATYP|ADDR|PORT`，atyp 1=IPv4/3=域名/4=IPv6）、`Network(0x04)`（2B，TCP=0）；读 4 字节响应头（`VER|STATUS|FEALEN(2)`），校验 `version==1 && status==0`（非 OK 状态按 go-gost 含义报错）后丢弃响应特征再透传。`tls: true` 复用共享 TLS 层（`sni`/`skip-cert-verify`/`fingerprint`/`client-fingerprint`，SNI 缺省回退 server），`forward: true` 省略 `Addr` 特征让 relay 路由到其预设上游；server 保留为域名按需解析；仅 TCP（无 UDP relay）；`mux`(smux 多路复用) 与 mTLS 客户端证书（`certificate`/`private-key`）在 config 阶段显式拒绝。线格式逐字段对齐 mihomo `transport/gost`；真实 fake relay server 互通测试见 `tests/gost_relay_outbound.rs`（明文 / 鉴权 / TLS / 错误凭据拒绝） |
...
**解析但无数据面（导入显示 OK，跑不通）：** `masque / trusttunnel / openvpn / tailscale / mieru / sudoku / dns`。（wireguard 已补 TCP + UDP 数据面，见上表 wireguard 行；http 上游代理 / ssh / hysteria(v1) / gost-relay 已补 TCP 数据面，见上表 http、ssh、hysteria(v1)、gost-relay 行。）
