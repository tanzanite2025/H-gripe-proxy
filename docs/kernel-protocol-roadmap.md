...
| 8 | WireGuard / ~~ssr~~ / snell / anytls 数据面 | 视订阅而定 | 大（各自独立工程） | 中-高 | ✅ **anytls 已完成**。✅ **snell 已完成**。✅ **ssr 已完成**（完整三层栈：流密码 + 协议 + 混淆；5 cipher × 4 protocol × 3 obfs；TCP relay；独立 fake SSR server 互通测试 `tests/ssr_outbound.rs`；老式流密码 SSR 专用、不暴露给 SS 出站）。WireGuard 按实际订阅命中再排；anytls UDP 已补。padding 流量整形仍为后续 |
